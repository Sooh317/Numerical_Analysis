...
Bucher = [[0, 0, 0, 0, 0, 0, 0], 
          ...
          [None, 16/135, 0, 6656/12825, 28561/56430, -9/50, 2/55], #5次
...
def internal(_x, _y, _dy, h, flag):
    x, y, dy = _x, _y, _dy
    F = [[0] * 6 for _ in range(2)] # F_0 : delta y, F_1 : delta dy/dx
    for i in range(6):
        nx = x + h * Bucher[i][0]
        ny = y
        ndy = dy
        for j in range(1, 7):
            ny += Bucher[i][j] * F[0][j - 1]
            ndy += Bucher[i][j] * F[1][j - 1]
        for j in range(2):
